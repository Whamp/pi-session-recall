...
The standalone `psr index` command remains the sole evidence writer and change detector. Operators can run it directly or opt into a native per-user schedule with `psr auto-index install`. Scheduling code installs one native definition that invokes `psr index` for periodic updates and removes stale optimization definitions from older releases; it does not scan sessions or write recall state. `psr index --rebuild` remains the explicit command for replacing incompatible index state.
