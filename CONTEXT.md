...
The Recall database that immediately preceded the Active recall database. `psr rollback` can restore its pointer without rebuilding it, and maintenance never removes it automatically. An unversioned version 6 database requires release `8402107` after restoration.
