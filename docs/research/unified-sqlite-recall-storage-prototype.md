# Unified SQLite Recall storage prototype
...
The stable design is now one derived SQLite database, canonical JSONL for complete payloads, no HNSW, no zero-vector tool rows, and no routine whole-database optimization. Sanitized measurements are in [`unified-sqlite-recall-storage-prototype-results.json`](unified-sqlite-recall-storage-prototype-results.json).
