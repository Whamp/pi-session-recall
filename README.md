...
Search costs rise with corpus size. The raw-agent evaluation used 15 synthetic files totaling 44,782 bytes. For context, the maintainer's session history measured on 2026-08-16 contained 3,803 files and 3.476 GiB of raw JSONL. Corpus sizes vary, but this example had 254 times as many files and more than 83,000 times as many bytes as the evaluation set. Its largest file was 130.66 MiB. At that scale, a fresh agent must repeatedly narrow paths, search text, parse records, and resolve competing evidence. Recall does that work during indexing and reuses the result across questions.

In one comparison on the maintainer's history, the full hybrid tool took 1.48 seconds at the median. A fresh agent restricted to raw JSONL took 94.43 seconds, examined 54 project files, and used 141,682 tokens plus 852,480 cached tokens. The agent found the answer reliably; the tool needed its maximum ten results to include the answer at rank ten. Indexed recall was about 64 times faster and far cheaper in this case, while the ranking result exposed work still needed. This is one measured query, not a universal quality or capacity claim. See [Production recall index value benchmark](docs/research/production-recall-index-value-benchmark.md).
