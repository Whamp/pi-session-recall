...
The standalone `psr index` command remains the sole evidence writer and change detector. Operators can run it directly or opt into a native per-user schedule with `psr auto-index install`. Scheduling code only installs or removes native definitions that invoke `psr index` for periodic updates and, with explicit `--optimize-daily` consent, `psr optimize` for daily collection optimization; it does not scan sessions or write recall state. `psr index --rebuild` remains the explicit command for replacing incompatible index state.
