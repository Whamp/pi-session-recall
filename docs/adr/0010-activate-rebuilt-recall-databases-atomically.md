...
The first generation rebuild treats a complete unversioned database in the recall data directory as the previous database. The unversioned files stay in place throughout the rebuild. This lets existing installations keep searching the old database during the build. The pointer can roll back after activation; restoring the unversioned version 6 database also requires redeploying release `8402107` before search or indexing resumes.

Failed and interrupted candidates stay outside the active path. `psr index --rebuild --stage --resume` continues the sole interrupted candidate and preserves its completed per-session catalog transactions and dense rows. Resume refuses zero or multiple candidates rather than guessing. The certified compact generation was built before ADR-0013 removed that one-time cutover path.
