...
The worker exits when no eligible work remains. It is not a daemon, watcher, process lease, or source of global leaf authority. Concurrent runtimes contribute a monotonic union of observed context exits.

## Rebuild, recovery, and rollback

A rebuild freezes incremental commits but not marker publication or search. It builds and optimizes a replacement generation beside the active generation. A short lock protects the checksummed pointer replacement. The ordinary worker then replays retained generation-independent markers against the replacement. The former active generation remains as bounded rollback material.

There are no dual-generation incremental writes. `/pi-session-recall-index --rollback` explicitly restores the retained generation and republishes retained marker work. `/pi-session-recall-index --collect-retired` removes only generations that are no longer active, building, replay-pending, or retained for rollback. Exact version-5 legacy layout adoption is explicit through `--adopt-legacy`; adoption serves the old generation read-only until a replacement build succeeds.
