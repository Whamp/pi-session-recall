...
A marker carries one runtime sequence and physical-session identity. Its marker ID is the canonical replay identity, including for markers that arrive below a runtime-sequence checkpoint. Clean-departure markers also capture the event-time logical session and leaf; empty tree transitions publish no marker. The worker orders and coalesces activity while preserving every branch exit.

The worker performs metadata-only recovery sweeps with durable per-directory checkpoints. Before declaring a sweep complete, it rescans completed directories so a file inserted while the sweep descended into a child cannot be skipped. Normal append processing validates the append cursor and its 4 KiB boundary fingerprint, opens the source once, reads every bounded range through that handle, and compares handle metadata before and after the read. It never combines path metadata with bytes from a later open. Projection payloads contain scalar IDs, links, boundaries, and spans; payloads above 1 MiB require explicit reconciliation.
...
Confirmed deletion records two durable acknowledgement phases in its tombstone. Recovery persists `ACK_PENDING` before acknowledging deletion markers and persists `ACKNOWLEDGED` before removing the physical projection. It discovers tombstones from durable projection state rather than relying on a caller's current source inventory. Recovery retries acknowledgement from `ACK_PENDING`; it resumes physical removal from `ACKNOWLEDGED` without acknowledging twice.
