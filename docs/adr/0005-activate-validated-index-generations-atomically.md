...
Exact version-5 legacy adoption remains explicit and read-only. Its next rebuild creates a managed self-contained generation. Interactive startup, lifecycle hooks, recall search, and source-neighborhood expansion never start a full build or report build progress in the TUI.
