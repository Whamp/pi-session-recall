...
/** Counts from temporary legacy-v6 rollback-window indexing. */
export interface LegacyV6ConversationIndexSummary {
...
/** Dependencies required to update an actual manifest-v6 Zvec database during rollback. */
export interface LegacyV6IncrementalSessionIndexerOptions {
...
function createLegacyV6ConversationIndexSummary(): LegacyV6ConversationIndexSummary {
  return {
    scannedSessions: 0,
    indexedSessions: 0,
    removedSessions: 0,
...
function emitMaintenanceWorksetProgress(
  options: LegacyV6IncrementalSessionIndexerOptions,
...
/** Updates only an actual legacy-v6 Zvec database and its index-state.json rollback data. */
export async function indexChangedLegacyV6ConversationSessions(
  options: LegacyV6IncrementalSessionIndexerOptions,
): Promise<LegacyV6ConversationIndexSummary> {
  ...
  const summary = createLegacyV6ConversationIndexSummary();
