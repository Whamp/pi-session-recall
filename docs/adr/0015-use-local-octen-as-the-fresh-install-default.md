...
Fresh `psr setup` installations default to `Octen/Octen-Embedding-0.6B` running in-process. macOS arm64 uses pinned native `onnxruntime-node` 1.27.0; Linux x64 and macOS x64 use pinned single-threaded `onnxruntime-web` 1.27.0 WASM. Stable `onnxruntime-node` packages do not ship a Darwin x64 binding, and native Intel Linux/macOS CI produced incompatible 0.73689-cosine vectors for the accepted quantized graph while WASM reached 0.98489. The certified artifact is a 1.01 GiB SmoothQuant INT8 ONNX conversion published in the project-controlled GitHub release [`model-octen-embedding-0.6b-onnx-int8-v1`](https://github.com/Whamp/pi-session-recall/releases/tag/model-octen-embedding-0.6b-onnx-int8-v1). The CLI verifies every file by byte size and SHA-256 before atomically making the complete model directory available.
