...
void test('legacy-v6 output preserves production hybrid and tool provenance formatting', () => {
  const legacyTool = {
    ...result,
    ...
    evidencePart: 'result' as const,
    role: 'tool' as const,
    toolName: 'read',
    toolCallId: 'legacy-call',
    toolCallEntryId: { value: 'legacy-call-entry' },
    toolResultEntryId: { value: 'legacy-result-entry' },
    toolError: true,
  };
  const output = formatRecallSearchResults({
    totalChunks: 42,
  ...
  assert.match(
    output,
    /across 42 indexed evidence documents with deterministic fusion v2 \(RRF k=60\)/u,
  );
  assert.match(output, /tool_result\/result · read · call legacy-call · error/u);
  assert.match(output, /Call source: legacy-call-entry · Result source: legacy-result-entry/u);
});
