...
A later [direct profile comparison](../research/local-vs-http-embedding-profile-comparison.md) found near-tied dense ranking quality between local Octen 0.6B and HTTP Octen 4B on 32 fixed query variants. The HTTP profile preserved all 32 sources in the final five, versus 31 for local, and projected to finish the current memoized full-rebuild embedding workload in 6.90 hours versus 20.72 hours locally. This supports keeping Will's GPU-backed HTTP deployment while retaining local Octen as the offline fresh-install default.
