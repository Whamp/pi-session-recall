...
`RecallConversationService` treats an injected profile as authoritative for manifest identity and zvec dimensions. The legacy configuration fields continue to create the existing Octen profile when no profile is injected. EmbeddingGemma extends manifest version 5 with optional fields; the Octen projection remains unchanged, so existing Octen generations remain readable.

EmbeddingGemma chunking uses the tokenizer inside the checksum-pinned GGUF. The embedded provider loads node-llama-cpp dynamically, verifies the model cache first, runs with CPU offload disabled, and L2-normalizes every vector. The service rejects non-repeatable query canaries before it writes a manifest or opens a new store.
