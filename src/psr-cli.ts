...
function formatRecallIndexProgress(
  ...
  switch (event.kind) {
    ...
    case 'physical-session-file-profiled': {
      ...
      const documentPhases = event.documentPhaseElapsedMilliseconds;
      return `  Profile: ${event.sessionPath} · ${formatPhysicalSessionSourceByteChange(event)} · ${ENGLISH_INTEGER_FORMAT.format(event.denseDocuments)} dense documents · ${ENGLISH_INTEGER_FORMAT.format(event.invocations)} Invocations · ${ENGLISH_INTEGER_FORMAT.format(event.newlyEmbeddedDocuments)} embedded · ${ENGLISH_INTEGER_FORMAT.format(event.reusedVectors)} reused · total ${formatProfileDuration(event.totalElapsedMilliseconds)} · read/parse ${formatProfileDuration(phases.readParse)} · graph validation ${formatProfileDuration(phases.graphValidation)} · documents/tokenization ${formatProfileDuration(phases.documentConstructionTokenization)} [pending documents ${formatProfileDuration(documentPhases.pendingAtomicSummaryDocuments)}; turn contexts ${formatProfileDuration(documentPhases.turnContextConstructionBudgetSplitting)}; chunk tokenization ${formatProfileDuration(documentPhases.conversationChunkTokenization)}; metadata/Invocations/Project ${formatProfileDuration(documentPhases.metadataInvocationProjectAttribution)}] · vector lookup ${formatProfileDuration(phases.vectorLookup)} · embedding ${formatProfileDuration(phases.embedding)} · SQLite replacement ${formatProfileDuration(phases.sqliteReplacement)}`;
    }
