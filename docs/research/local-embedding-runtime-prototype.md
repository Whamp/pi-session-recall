...
Use `Octen/Octen-Embedding-0.6B` as the default local embedding model for new Pi Session Recall installations. Run the verified SmoothQuant INT8 ONNX artifact through native `onnxruntime-node` 1.27.0 on macOS arm64 and Linux x64 AMD processors. Use `onnxruntime-web` 1.27.0 WASM on Linux x64 Intel or unknown processors and macOS x64 because native Intel execution produced incompatible vectors. Store native 1,024-dimensional normalized vectors.
