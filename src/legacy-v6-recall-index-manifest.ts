...
export interface LegacyV6RecallEmbeddingModelIdentity {
  requestModel: string;
...
export interface LegacyV6RecallTokenizerManifestIdentity {
  model: string;
  revision: string;
...
/** Exact identity of one temporary legacy-v6 rollback Zvec database. */
export interface LegacyV6RecallIndexManifest {
  manifestVersion: 6;
  importPolicy: { version: number };
  embedding: LegacyV6RecallEmbeddingModelIdentity;
  tokenizer: LegacyV6RecallTokenizerManifestIdentity;
  chunkPolicy: {
    version: 3;
    maxTokens: number;
...
const legacyV6RecallIndexManifestSchema = Type.Object(
  {
...
function createLegacyV6TokenizerManifestIdentity(
  identity: ConversationTokenizerAssetIdentity,
): LegacyV6RecallTokenizerManifestIdentity {
  return {
...
/** Creates the exact actual-v6 identity used to validate rollback search and indexing. */
export function createLegacyV6RecallIndexManifest(options: {
  embeddingIdentity: LegacyV6RecallEmbeddingModelIdentity;
  tokenizerIdentity?: LegacyV6RecallTokenizerManifestIdentity;
  chunkPolicy?: RecallChunkPolicy;
  projectLineages?: RecallProjectLineages;
}): LegacyV6RecallIndexManifest {
  ...
  return {
    manifestVersion: LEGACY_V6_RECALL_INDEX_MANIFEST_VERSION,
    ...
    tokenizer: structuredClone(
      options.tokenizerIdentity ??
        createLegacyV6TokenizerManifestIdentity(OCTEN_TOKENIZER_IDENTITY),
    ),
    chunkPolicy: {
      version: 3,
...
export function assertLegacyV6RecallIndexManifestCompatible(
  actual: LegacyV6RecallIndexManifest | null,
  expected: LegacyV6RecallIndexManifest,
  manifestPath: string,
): asserts actual is LegacyV6RecallIndexManifest {
  if (!actual) {
    throw new Error(
...
/** Reads and strictly validates an actual production version-6 manifest. */
export async function readLegacyV6RecallIndexManifest(
  manifestPath: string,
): Promise<LegacyV6RecallIndexManifest | null> {
  let content: string;
  try {
  ...
  try {
    ...
    return Value.Parse(legacyV6RecallIndexManifestSchema, parsed);
  } catch (error) {
...
/** Writes an actual version-6 manifest only while constructing legacy compatibility fixtures. */
export async function writeLegacyV6RecallIndexManifest(
  manifestPath: string,
  manifest: LegacyV6RecallIndexManifest,
): Promise<void> {
  Value.Parse(legacyV6RecallIndexManifestSchema, manifest);
