...
The Pi extension serves model-facing recall reads and publishes small immutable recall work markers. It registers no index-maintenance slash command and sends no maintenance status or progress messages to the TUI. A standalone `pi-session-recall` CLI is the sole operator control surface for setup, status, explicit catch-up, rebuild control, recovery, rollback, legacy adoption, and cleanup. The current slash command is transitional implementation debt, not an accepted interface; the CLI migration removes it rather than retaining it as a wrapper. ADR 0004's short-lived external worker handles deferred incremental ingestion outside Pi.
