...
The index manifest records the import-policy version. Incremental state records the same policy under state schema version 3. Older manifests and state files require `psr index --rebuild`; the target architecture does not adopt or migrate them.
