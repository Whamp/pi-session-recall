...
export function assertRecallIndexManifestCompatible(
  ...
  if (!actual) {
    throw new Error(
      `Recall index manifest missing at ${manifestPath}; create a fresh generation with pi-session-recall rebuild`,
    );
  }
  ...
  if (mismatches.length > 0) {
    throw new Error(
      `Recall index manifest incompatible at ${manifestPath}:\n- ${mismatches.join('\n- ')}\nCreate a fresh generation with pi-session-recall rebuild.`,
    );
  }
}
