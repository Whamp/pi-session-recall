...
| Embedding profile change requires staging and never mixes vectors                                              | `recall-inference-configuration.test.ts:141`; `recall-conversation-service.test.ts:3860`; profile-bound selector checks in `recall-index-generations.ts`       | Pass — deterministic                           |
...
| Active generation remains searchable during replacement                                                        | `recall-conversation-service.test.ts:3860`; `recall-background-index-conversation-service.test.ts:137`                                                         | Pass — integration                             |
| Only a complete validated staging generation activates                                                         | `recall-conversation-service.test.ts:3860,4055`; phase interruption test at `recall-background-index-conversation-service.test.ts:391`                         | Pass — integration                             |
...
| Existing Octen configuration/manifest needs no rebuild                                      | `recall-index-manifest.test.ts:75`; unchanged Octen evidence hashes in `embedded-profile-acceptance.json` | Pass — deterministic              |
...
| Acceptance criterion                                                     | Evidence                                                                               | Status                            |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- | --------------------------------- |
| Generation owns store/state/manifest and one embedding profile           | `recall-index-generations.ts`; ADR 0005                                                | Pass — observable + deterministic |
| Replacement writes staging while search reads active                     | `recall-conversation-service.test.ts:3860`                                             | Pass — integration                |
| Staging catches changed files before activation                          | `recall-conversation-service.test.ts:3860`                                             | Pass — integration                |
| Manifest/canary/dimensions/counts/state/store health validate first      | rebuild validation path in `recall-conversation-service.ts`; failure test at line 4055 | Pass — integration                |
| Store optimizes before one atomic selector switch                        | rebuild path and activation test                                                       | Pass — integration                |
| Interruption/failure preserves active and resumable staging              | `recall-conversation-service.test.ts:3971,4055`                                        | Pass — integration                |
| Explicit discard removes staging only                                    | `recall-conversation-service.test.ts:4055`; index-command control test                 | Pass — integration                |
| Interactive Pi lifecycle/search does not start whole-session maintenance | `recall-extension.test.ts:65`; README explicit-maintenance contract                    | Pass — extension contract         |
...
| Parsing/embedding/store-write/optimization/pre-activation interruptions recover | parameterized test at line 391                                     | Pass — child process integration |
...
| Sample embeddings persist in profile cache and full build reuses them           | `recall-first-index-setup.test.ts:180`                          | Pass — integration                          |
...
| Acceptance criterion                                                                             | Evidence                                                                     | Status                                             |
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | -------------------------------------------------- |
| Octen quality result unchanged and profile-scoped                                                | hashes in `embedded-profile-acceptance.json`                                 | Pass                                               |
...
| Background interruption/resumption/availability/catch-up/validation/activation exercised         | background and generation integration suites                                 | Pass — integration                                 |
...
Run from the issue #28 worktree:
...
git diff --check dc8c0dc8f9cdad9bf3a3ef4699375a7252e4dc1f...HEAD
