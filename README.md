...
That repeated work grows with the session repository. The raw-agent evaluation used 15 synthetic files totaling 44,782 bytes. For scale, the production history measured on 2026-08-16 contained 3,803 files and 3.476 GiB of raw JSONL—254 times as many files and more than 83,000 times as many bytes. Its largest file was 130.66 MiB. A fresh agent cannot put that whole corpus in model context; it must repeatedly narrow paths, search text, parse records, and resolve competing evidence. Recall pays those costs during indexing and reuses the result across questions.
