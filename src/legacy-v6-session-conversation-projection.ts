/**
 * Temporary rollback-only legacy-v6 session projection.
 *
 * This file preserves origin/master's complete lexical tool projection and is deletable with the
 * legacy-v6 adapter after the rollback window. Do not import it from version 8 indexing paths.
 */
...
export interface LegacyV6SessionConversationImport {
  format: SessionImportFormat;
  logicalSessions: LegacyV6SessionConversationLogicalSession[];
  chunks: LegacyV6SessionConversationChunk[];
}

/** A token-bounded recall evidence document with complete source and session-graph provenance. */
export interface LegacyV6SessionConversationChunk {
  ...
  sessionName: string;
  entryId: LegacyV6PiSessionEntryId;
  parentEntryId: LegacyV6PiSessionEntryId | null;
  childEntryIds: LegacyV6PiSessionEntryId[];
  contributingEntryIds: LegacyV6PiSessionEntryId[];
  currentLeafId: LegacyV6PiSessionEntryId | null;
  branchPathLeafIds: LegacyV6PiSessionEntryId[];
  isOnActiveBranch: boolean;
  isVisibleInActiveContext: boolean;
  compactedByEntryIds: LegacyV6PiSessionEntryId[];
  compactionFirstKeptEntryId: LegacyV6PiSessionEntryId | null;
  branchSummaryFromEntryId: LegacyV6PiSessionEntryId | null;
  ...
  toolCallEntryId: LegacyV6PiSessionEntryId | null;
  toolResultEntryId: LegacyV6PiSessionEntryId | null;
...
interface PendingConversationDocumentBase {
  ...
  role: LegacyV6SessionConversationChunk['role'];
  summaryKind: LegacyV6SessionConversationChunk['summaryKind'];
  evidenceKind: LegacyV6SessionConversationChunk['evidenceKind'];
  evidencePart: LegacyV6SessionConversationChunk['evidencePart'];
  isDenseSearchable: boolean;
...
interface LegacyV6SessionConversationChunkContext {
...
function createLegacyV6PiSessionEntryId(value: string): LegacyV6PiSessionEntryId {
  return { value };
}
...
function createLegacyV6SessionConversationChunks(
  context: LegacyV6SessionConversationChunkContext,
  pending: PendingConversationDocument,
): LegacyV6SessionConversationChunk[] {
  const { graph } = context;
  const entryId = createLegacyV6PiSessionEntryId(pending.entry.id);
  const contributingEntryIds = pending.contributingEntries.map((entry) =>
    createLegacyV6PiSessionEntryId(entry.id),
  );
  ...
  const textRunId = hashConversationValue(
    `${LEGACY_V6_SESSION_CONVERSATION_SCHEMA_VERSION}\0${context.logicalSessionIdentity}\0${pending.entry.id}\0${contributingEntryIdentity}\0${pending.evidenceKind}\0${pending.evidencePart}\0${pending.textRun.textRunIndex}`,
  ).slice(0, 40);
  ...
  const chunkIds = chunkSpans.map((span) =>
    hashConversationValue(
      `${LEGACY_V6_SESSION_CONVERSATION_SCHEMA_VERSION}\0${textRunId}\0${span.characterStart}\0${span.characterEnd}`,
    ).slice(0, 40),
  );
  ...
  return chunkSpans.map((span, chunkIndex) => {
    ...
    return {
      schemaVersion: LEGACY_V6_SESSION_CONVERSATION_SCHEMA_VERSION,
      ...
      parentEntryId: pending.entry.parentId
        ? createLegacyV6PiSessionEntryId(pending.entry.parentId)
        : null,
      childEntryIds: (graph.childEntryIdsById.get(pending.entry.id) ?? []).map(
        createLegacyV6PiSessionEntryId,
      ),
      contributingEntryIds,
      currentLeafId: context.currentLeafId,
      branchPathLeafIds: branchPathLeafIds.map(createLegacyV6PiSessionEntryId),
      ...
      compactedByEntryIds: compactedByEntryIds.map(createLegacyV6PiSessionEntryId),
      compactionFirstKeptEntryId: pending.compactionFirstKeptEntryId
        ? createLegacyV6PiSessionEntryId(pending.compactionFirstKeptEntryId)
        : null,
      branchSummaryFromEntryId: pending.branchSummaryFromEntryId
        ? createLegacyV6PiSessionEntryId(pending.branchSummaryFromEntryId)
        : null,
      role: pending.role,
      ...
      toolCallEntryId: pending.toolCallEntryId
        ? createLegacyV6PiSessionEntryId(pending.toolCallEntryId)
        : null,
      toolResultEntryId: pending.toolResultEntryId
        ? createLegacyV6PiSessionEntryId(pending.toolResultEntryId)
        : null,
...
function createLogicalSessionSummary(
  session: CanonicalSessionRepresentation,
  graph: ParsedSessionGraph,
): LegacyV6SessionConversationLogicalSession {
  return {
 ...
 */
export async function readLegacyV6SessionConversationImport(
  sessionPath: string,
  options: LegacyV6SessionConversationChunkOptions,
): Promise<LegacyV6SessionConversationImport> {
  ...
  const logicalSessions: LegacyV6SessionConversationLogicalSession[] = [];
  const chunks: LegacyV6SessionConversationChunk[] = [];
  for (const session of imported.sessions) {
    ...
    const logicalSessionIdentity =
      imported.format === SessionImportFormat.PI_SESSION_REUSE_HISTORY
        ...
        : graph.header.id;
    const context: LegacyV6SessionConversationChunkContext = {
      graph,
      sessionPath,
      ...
      currentLeafId: graph.currentLeafId
        ? createLegacyV6PiSessionEntryId(graph.currentLeafId)
        : null,
    };
    chunks.push(
      ...createPendingConversationDocuments(graph)
        ...
        .flatMap((pending) => createLegacyV6SessionConversationChunks(context, pending)),
    );
  }
 ...
 */
export async function readLegacyV6SessionConversationChunks(
  sessionPath: string,
  options: LegacyV6SessionConversationChunkOptions,
): Promise<LegacyV6SessionConversationChunk[]> {
  return (await readLegacyV6SessionConversationImport(sessionPath, options)).chunks;
}
